...
class SimCLRLoss(LossFunctionBase):
    ...
    def __init__(
        ...
        self.neck = NonLinearNeckSimCLR(in_channels=model_output_dim,
    ...
    def _create_buffer(self, N, device):
        ...
        pos_ind = (torch.arange(N * 2).to(device),
    ...
    def forward(self, model, inp, **kwargs):
        ...
        """
        assert inp.dim() == 5, \
            "Input must have 5 dims, got: {}".format(inp.dim())
        # reshape batch dimension to work with backbone
        inp = inp.reshape(
            inp.size(0) * 2, inp.size(2), inp.size(3), inp.size(4))
        x = model(inp)  # 2n
