...
MODEL_LAYERS = {
    ...
    + ["classifier.2"]
    + ["classifier.5"],
    "alexnet_64x64_rl_scratch_truncated": ["features.2"]
    + ["features.5"]
    + ["features.7"]
    + ["features.9"],
    ...
    + ["classifier.3"]
    + ["classifier.7"],
    "alexnet_two_64x64": ["features.2"] + ["features.5"],
    "alexnet_three_64x64": ["features.2"] + ["features.5"] + ["features.7"],
    "alexnet_four_64x64": ["features.2"]
    + ["features.5"]
    + ["features.7"]
    + ["features.9"],
    "alexnet_five_64x64": ["features.2"]
    + ["features.5"]
    + ["features.7"]
    + ["features.9"]
    + ["features.12"],
    "alexnet_six_64x64": ["features.2"]
    + ["features.5"]
    + ["features.7"]
    + ["features.9"]
    + ["features.12"]
    ...
    "squeezenet1_0": [
        "features." + layer
        for layer in ["2"]
        + [f"{i}.expand3x3_activation" for i in [3, 4, 5, 7, 8, 9, 10, 12]]
    ],
    "squeezenet1_1": [
        "features." + layer
        for layer in ["2"]
        + [f"{i}.expand3x3_activation" for i in [3, 4, 6, 7, 9, 10, 11, 12]]
    ],
    # DenseNets
    "densenet121": ["features.pool0"]
    + [f"features.denseblock1.denselayer{i+1}" for i in range(6)]
    + ["features.transition1.pool"]
    + [f"features.denseblock2.denselayer{i+1}" for i in range(12)]
    + ["features.transition2.pool"]
    + [f"features.denseblock3.denselayer{i+1}" for i in range(24)]
    + ["features.transition3.pool"]
    + [f"features.denseblock4.denselayer{i+1}" for i in range(16)],
    "densenet161": ["features.pool0"]
    + [f"features.denseblock1.denselayer{i+1}" for i in range(6)]
    + ["features.transition1.pool"]
    + [f"features.denseblock2.denselayer{i+1}" for i in range(12)]
    + ["features.transition2.pool"]
    + [f"features.denseblock3.denselayer{i+1}" for i in range(36)]
    + ["features.transition3.pool"]
    + [f"features.denseblock4.denselayer{i+1}" for i in range(24)],
    "densenet169": ["features.pool0"]
    + [f"features.denseblock1.denselayer{i+1}" for i in range(6)]
    + ["features.transition1.pool"]
    + [f"features.denseblock2.denselayer{i+1}" for i in range(12)]
    + ["features.transition2.pool"]
    + [f"features.denseblock3.denselayer{i+1}" for i in range(32)]
    + ["features.transition3.pool"]
    + [f"features.denseblock4.denselayer{i+1}" for i in range(32)],
    "densenet201": ["features.pool0"]
    + [f"features.denseblock1.denselayer{i+1}" for i in range(6)]
    + ["features.transition1.pool"]
    + [f"features.denseblock2.denselayer{i+1}" for i in range(12)]
    + ["features.transition2.pool"]
    + [f"features.denseblock3.denselayer{i+1}" for i in range(48)]
    + ["features.transition3.pool"]
    + [f"features.denseblock4.denselayer{i+1}" for i in range(32)],
    # Inceptions
    "googlenet": ["maxpool1", "maxpool2"]  # Inception v1
    ...
    # ShuffleNet
    "shufflenet_v2_x0_5": ["maxpool"]
    + [f"stage2.{i}" for i in range(4)]
    + [f"stage3.{i}" for i in range(8)]
    + [f"stage4.{i}" for i in range(4)]
    + ["conv5.2"],
    "shufflenet_v2_x1_0": ["maxpool"]
    + [f"stage2.{i}" for i in range(4)]
    + [f"stage3.{i}" for i in range(8)]
    + [f"stage4.{i}" for i in range(4)]
    + ["conv5.2"],
    # MobileNet
    "mobilenet_v2": ["features.0.2"]
    + ["features.1.conv.1"]
    + [f"features.{i}.conv.2" for i in range(2, 18)]
    + ["features.18.2"],  # output conv of each inverted res block
    # MNASNets
    "mnasnet0_5": ["layers.2", "layers.5"]
    ...
    "simplified_mousenet_single_stream": ["VISp", "ventral", "VISpor"],
}
...
for path in [1, 2]:
    for block_idx in range(0, 10):
        CPC_2P_MODEL_LAYERS.append(
            f"backbone.path{path}.res_blocks.res{block_idx}.branch2.a_relu"
        )
        CPC_2P_MODEL_LAYERS.append(
            f"backbone.path{path}.res_blocks.res{block_idx}.branch2.b_relu"
        )
        CPC_2P_MODEL_LAYERS.append(
            f"backbone.path{path}.res_blocks.res{block_idx}.relu"
        )
CPC_2P_MODEL_LAYERS.append(
    "relu1"
)  # after features from both paths are concatenated, avg pooled and relu
...
MODEL_LAYERS["alexnet_bn_64x64_input_pool_6_with_ir_transforms"] = MODEL_LAYERS[
    "alexnet"
]
MODEL_LAYERS["alexnet_64x64_input_pool_1"] = MODEL_LAYERS["alexnet"]
MODEL_LAYERS["alexnet_64x64_input_dict"] = [
    "pool1",
    "pool2",
    "conv3",
    "conv4",
...
MODEL_LAYERS["alexnet_bn_barlow_twins_64x64"] = MODEL_LAYERS["alexnet_bn"]
MODEL_LAYERS["alexnet_bn_vicreg_64x64"] = MODEL_LAYERS["alexnet_bn"]
...
MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3_ir_32x32"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3_ir_44x44"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3_ir_84x84"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3_ir_104x104"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3_ir_124x124"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3_ir_144x144"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3_ir_164x164"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3_ir_184x184"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3_ir_204x204"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]

# 224 px models
MODEL_LAYERS["simplified_mousenet_single_stream_ir_224x224"] = MODEL_LAYERS[
    "simplified_mousenet_single_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3_ir_224x224"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_six_stream_visp_3x3_ir_224x224"] = MODEL_LAYERS[
    "simplified_mousenet_six_stream"
]

# 64 px models
MODEL_LAYERS["simplified_mousenet_six_stream_visp_3x3"] = MODEL_LAYERS[
    "simplified_mousenet_six_stream"
]
MODEL_LAYERS["simplified_mousenet_six_stream_visp_3x3_bn"] = MODEL_LAYERS[
    "simplified_mousenet_six_stream"
]
MODEL_LAYERS["simplified_mousenet_six_stream_vispor_only"] = MODEL_LAYERS[
    "simplified_mousenet_six_stream"
]
MODEL_LAYERS["simplified_mousenet_six_stream_vispor_only_visp_3x3"] = MODEL_LAYERS[
    "simplified_mousenet_six_stream"
]

MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3_bn"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_vispor_only"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_vispor_only_visp_3x3"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]

MODEL_LAYERS["simplified_mousenet_six_stream_cifar10"] = MODEL_LAYERS[
    "simplified_mousenet_six_stream"
]
MODEL_LAYERS["simplified_mousenet_six_stream_visp_3x3_cifar10"] = MODEL_LAYERS[
    "simplified_mousenet_six_stream"
]
MODEL_LAYERS["simplified_mousenet_six_stream_vispor_only_cifar10"] = MODEL_LAYERS[
    "simplified_mousenet_six_stream"
]
MODEL_LAYERS[
    "simplified_mousenet_six_stream_vispor_only_visp_3x3_cifar10"
] = MODEL_LAYERS["simplified_mousenet_six_stream"]
MODEL_LAYERS["simplified_mousenet_six_stream_visp_3x3_bn_cifar10"] = MODEL_LAYERS[
    "simplified_mousenet_six_stream"
]
MODEL_LAYERS["simplified_mousenet_six_stream_visp_3x3_ir"] = MODEL_LAYERS[
    "simplified_mousenet_six_stream"
]
MODEL_LAYERS["simplified_mousenet_six_stream_visp_3x3_rotnet"] = MODEL_LAYERS[
    "simplified_mousenet_six_stream"
]
MODEL_LAYERS["simplified_mousenet_six_stream_visp_3x3_simclr"] = MODEL_LAYERS[
    "simplified_mousenet_six_stream"
]
MODEL_LAYERS["simplified_mousenet_six_stream_visp_3x3_mocov2"] = MODEL_LAYERS[
    "simplified_mousenet_six_stream"
]
MODEL_LAYERS["simplified_mousenet_six_stream_visp_3x3_simsiam"] = MODEL_LAYERS[
    "simplified_mousenet_six_stream"
]
MODEL_LAYERS["simplified_mousenet_ae_six_stream"] = MODEL_LAYERS[
    "simplified_mousenet_six_stream"
]
MODEL_LAYERS["simplified_mousenet_depth_hour_glass_six_stream"] = MODEL_LAYERS[
    "simplified_mousenet_six_stream"
]

MODEL_LAYERS["simplified_mousenet_dual_stream_cifar10"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3_cifar10"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_vispor_only_cifar10"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS[
    "simplified_mousenet_dual_stream_vispor_only_visp_3x3_cifar10"
] = MODEL_LAYERS["simplified_mousenet_dual_stream"]
MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3_bn_cifar10"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3_ir"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3_rotnet"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3_simclr"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3_mocov2"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_dual_stream_visp_3x3_simsiam"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_ae_dual_stream"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]
MODEL_LAYERS["simplified_mousenet_depth_hour_glass_dual_stream"] = MODEL_LAYERS[
    "simplified_mousenet_dual_stream"
]

MODEL_LAYERS["simplified_mousenet_single_stream_cifar10"] = MODEL_LAYERS[
    "simplified_mousenet_single_stream"
]
MODEL_LAYERS["simplified_mousenet_single_stream_ir"] = MODEL_LAYERS[
    "simplified_mousenet_single_stream"
]
MODEL_LAYERS["simplified_mousenet_single_stream_rotnet"] = MODEL_LAYERS[
    "simplified_mousenet_single_stream"
]
MODEL_LAYERS["simplified_mousenet_single_stream_simclr"] = MODEL_LAYERS[
    "simplified_mousenet_single_stream"
]
MODEL_LAYERS["simplified_mousenet_single_stream_mocov2"] = MODEL_LAYERS[
    "simplified_mousenet_single_stream"
]
MODEL_LAYERS["simplified_mousenet_single_stream_simsiam"] = MODEL_LAYERS[
    "simplified_mousenet_single_stream"
]
MODEL_LAYERS["simplified_mousenet_ae_single_stream"] = MODEL_LAYERS[
    "simplified_mousenet_single_stream"
]
MODEL_LAYERS["simplified_mousenet_depth_hour_glass_single_stream"] = MODEL_LAYERS[
    "simplified_mousenet_single_stream"
]
...
for area in ["VISp", "VISal", "VISpl", "VISli", "VISrl", "VISl", "VISpor"]:
    MODEL_LAYER_CONCATS["shi_mousenet"]["1"][area] = [
        area + cortical for cortical in ["4", "2/3", "5"]
    ]
